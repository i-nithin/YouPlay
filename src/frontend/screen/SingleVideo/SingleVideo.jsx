...
import { combinedService } from "../../services/combainedServices";
...
function SingleVideo() {
  ...
  const saveToWatchLater = async () => {
    const data = await combinedService(
      "post",
      "/api/user/watchlater",
      isAuth.token,
      video
  ...
  return (
    <>
          ...
          <div className="single-video-container">
            <div className="video-container">
              ...
              <div className="video-body">
                <div className="single-video-title">{title}</div>
                <div className="d-flex justify-between">
                  <div className="video-card-channel-title">
                    {numFormatter(views)} |{" "}
                    {timeFormatter(Date.parse(createdAt))} ago
                  </div>
                  <div className="d-flex gap-1 ">
                    {likesData.find(
                      (singlevideo) => singlevideo._id === videoId
                    ) ? (
                      <div
                        className="single-video-title-icons"
                ...
                <div className="single-video-details d-flex gap-1">
                  <img
                    src={channelAvtar}
                    alt="channel img"
                    className="br-round channel-img"
                  />
                  <div>
                    <div>{channel}</div>
            ...
            <div className="video-recomendation-list-container">
              {videos.map((video) => {
                return <SmallVideoCard key={video._id} video={video} />;
              })}
            </div>
          </div>
        </div>
      ...
      {modal && (
        <PlaylistModal modal={modal} setmodal={setModal} video={video} />
      )}
    </>
  );
}
